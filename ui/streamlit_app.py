...
st.set_page_config(page_title="DevCopilot AI", layout="wide")

# Sidebar
with st.sidebar:
    st.image("https://1lm.me/cc.png", width=180)
    st.title("DevCopilot AI")
    ...
    st.markdown("---")
    st.markdown("Made with ❤️ by DevCopilot")
...
if selected == "Analyze Repo":
    ...
    if submit:
        ...
        if not backend_is_ready():
            st.error(f"🚫 DevCopilot backend not reachable at {API_URL}. Please start the API.")
            st.stop()
    ...
    st.title("ℹ️ About DevCopilot AI")
    st.markdown("""
    **DevCopilot AI** is your autonomous programming assistant that can:
    ...
    Built with ❤️ using LangChain, LangGraph, FAISS, and Streamlit.
    """)